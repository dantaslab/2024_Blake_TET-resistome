...
covariates <- list(
    ...
    Age = "Age",
    Gender = "Gender",
    Stool_grade = "Stool_grade",
    Sample_code = "Sample_code",
    # Weeks = "Weeks",
    # Antibiotics = "antibiotics_in_last_week",
...
perindividual_covariates <- c(
    "Age", "Gender", "Traveler_type", "Subject_id"
    # "BMI_imputed", "race_simplified", "site_name", "diagnosis",
...
put_in_heatmap <- c(
    "Age", "Gender", "Stool_grade", "Sample_code", "Disease",
    "Subject", "All")
...
# Load metadata
e00.cusco.metadata.df <- load_metadata(metadata.file)

e00.combined.df <- e00.cusco.metadata.df$combined
e00.subjects.df <- e00.cusco.metadata.df$subjects
e00.samples.df <- e00.cusco.metadata.df$samples
e00.gicases.df <- e00.cusco.metadata.df$gievents

# Load metaphlan data and filter taxa, if the relative abundance of taxa is 
# less than 0.01 in more than 90% of the samples
e01.cusco.metaphlan.df <- load_metaphlan_data(metaphln.relab.file)

e01.metaphlan.prev.df <- e01.cusco.metaphlan.df$metaphlan.prev
e01.cusco.relab.filtrd.df <- e01.cusco.metaphlan.df$metaphlan.relab

tmp.df <- filter_metaphlan(e01.cusco.relab.filtrd.df, relab = 0.01, prev = 0.1, var = 0.001)

# Convert to sample by species
r01.sam.by.specs.df <- dcast(tmp.df$metaphlan.relab, 
                                ID ~ Species, 
                                value.var = "RelativeAbundance",
                                fill = 0,
                                fun.aggregate = sum)

r01.sam.by.specs.df <- r01.sam.by.specs.df %>% 
                            filter(ID %in% e00.samples.df$Sample_id)

taxacol <- names(r01.sam.by.specs.df[ , 2:ncol(r01.sam.by.specs.df)])

r02.mphln.w.meta.df <- e00.combined.df %>% inner_join(r01.sam.by.specs.df, by = c("Sample_id" = "ID"))


pcl <- list(x = r02.mphln.w.meta.df[, c("Sample_id", taxacol)], 
    meta = r02.mphln.w.meta.df[, !colnames(r02.mphln.w.meta.df) %in% taxacol])
...
bc_omnibus_tests <- function(pcl, week_offset=0, method="bray", act_lenience=2, Nperms=4999) {
    ...
    if (week_offset == 0) {
        ...
        ok_covars <- (colMeans(is.na(pcl$meta[,covars.unl])) < 0.8)
    ...
    # keep <- !is.na(pcl$meta$Subject_id) & (pcl.apply.s(pcl, any(x>0)))
    # 
    # if (!all(keep)) {
    ...
    pcl$x <- pcl$x %>% column_to_rownames("Sample_id")
    
    # Get the distance matrix
    D <- vegdist(pcl$x, method)
    ...
    covars <- union(overall_covars, "Subject_id")
    
    # Make a per-person covariate table
    pcl$meta$Subject_id <- factor(pcl$meta$Subject_id)
    subj_meta <- pcl$meta[order(pcl$meta$Subject_id),,drop=F]
    subj_meta <- subj_meta[!duplicated(subj_meta$Subject_id), covars[covars %in% perindividual_covariates], drop=F]
    rownames(subj_meta) <- subj_meta$Subject_id
    
    rownames(pcl$meta) <- pcl$meta$Sample_id
    # permute_within <- pcl$meta %>% dplyr::select("Subject_id","Stool_grade", "Sample_code")
    permute_within <- pcl$meta %>% dplyr::select(overall_covars)
    ...
    for (var in names(covariates)) {
        ...
        if (any(ok_covars[varsub])) {
            ...
            if ("Subject_id" %in% varsub) {
                ...
                rownames(permute_within) <- mt$Sample_id
                block_data <- subj_meta_sub[,varsub[varsub %in% perindividual_covariates], drop=F]
                rownames(block_data) <- subj_meta_sub$Subject_id
                sep.ad <- PERMANOVA_repeat_measures(
                    D=Dsub, permutations=Nperms, na.rm=T,
                    #permute_within= mt[,varsub[!(varsub %in% perindividual_covariates)], drop=F],
                    permute_within = permute_within,
                    blocks=mt$Subject_id,
                    #block_data=subj_meta_sub[,varsub[varsub %in% perindividual_covariates], drop=F]
                    #block_data=(subj_meta_sub %>% dplyr::select(varsub))
...
# 
# We will determine whether gut resistomes of travelers from different countries converge over time to a shared "traveler" resistome, or whether their resistomes remain similar to their country of origin.
# 
# ```{r sec-2.4-args-beta-1, eval=FALSE, fig.height = 5, fig.width = 8, fig.align = "center"}
# 
# df <- e01.shortbred.markerfile %>% 
#     group_by(Marker_Source, AMR_Mechanism, AMR_Classification) %>%
#     summarise(counts = n())
# 
# df$AMR_Classification <- str_wrap(df$AMR_Classification, width="25")
# df$AMR_Mechanism <- str_wrap(df$AMR_Mechanism, width="25")
# 
# # p05 <- ggplot(e04.metadata.filtered, aes(x=factor(Weeks), y=(TotalRPKM))) + 
# #     geom_boxplot(notch=FALSE, color="black", alpha=0.7) + 
# #     # geom_point(aes(shape=factor(SampleCode),colour=SubjectID), size=2, alpha=0.9) +
# #     # geom_line(aes(group=SubjectID, colour=SubjectID), size=0.5, lty=1) +
# #     theme_pub() +
# #     theme(legend.title = element_blank(), legend.position="none") +
# #     facet_grid(TravelerType.x ~ ., scale="fixed") +
# #     labs(x="Weeks", y="log10(TotalRPKM)", title="", size=12)
# # 
# # # pairwise.t.test(e03_all_metadata.dat, Country_of_residence ~ RPKM)
# # # plt1 <- plt1 + stat_compare_means(label = "p.format", method="t.test")
# # p05
# 
# p01 <- ggplot(df, aes(x=reorder(str_to_title(AMR_Mechanism), counts, sum), y = counts)) +
#     geom_bar(aes(fill = Marker_Source), stat = "identity") +
#     # geom_text(aes(label = counts), vjust = -0.3) +
#     coord_flip() +
#     theme_pub() +
#     theme(legend.title = element_blank(), legend.direction = "horizontal", legend.box = "horizontal", legend.position="top") +
#     scale_fill_manual(values = c("#5ca8db", "#efb632")) +
#     # facet_wrap(~AR_Mechanism)
#     labs(x="Resistance Mechanisms", y="", title="")
# 
# p01
# 
# df <- e02.shortbred.markerfile %>% 
#     group_by(Marker_Source, AR_Mechanism, AR_Category) %>% 
#     summarise(counts = n())
# 
# df$AR_Category <- str_wrap(df$AR_Category,width="20")
# df$AR_Mechanism <- str_wrap(df$AR_Mechanism,width="20")
# 
# p02 <- ggplot(df, aes(x=reorder(str_to_title(AR_Mechanism), counts, sum), y = counts)) +
#     geom_bar(aes(fill = AR_Category), stat = "identity") +
#     # geom_text(aes(label = counts), vjust = -0.3) +
#     # theme_bw() +
#     # coord_flip() +
#     theme_pub() +
#     theme(legend.title = element_blank(), legend.direction = "horizontal", legend.box = "horizontal", legend.position="bottom") +
#     guides(fill= guide_legend(nrow = 3, bycol = TRUE)) +
#     # scale_fill_manual(values = c("#C6D166", "#EB5E30")) +
#     facet_wrap(~Marker_Source) +
#     labs(x="", y="", title="")
# 
# p02
# 
# # fname_pdf <- "res01_shortbred_markerinfo.pdf"
# # fname_png <- "res01_shortbred_markerinfo.png"
# # ggsave(fname_pdf, plot=ggarrange(p01, p02, ncol = 1, nrow = 2), path="Figures/pdf", width=550, height=350, units="mm")
# # ggsave(fname_png, plot=ggarrange(p01, p02, ncol = 1, nrow = 2), path="Figures/png", width=550, height=350, units="mm")
